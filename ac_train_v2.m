function [ac, ev_stats] = ac_train_v2(ac, ae, imdb, opts)
...
if opts.evaluate
    opts.epoch = start;
    [ac, ev_stats.train] = processEpoch_val(ac, ae, imdb, opts, 'train', f_val) ;
    [ac, ev_stats.val] = processEpoch_val(ac, ae, imdb, opts, 'val', f_val) ;
    return; 
end
...
for epoch=start+1:opts.numepochs
    ...
    [ac, stats{end,2}.val] = processEpoch_val(ac, ae, imdb, params, 'val', f_val) ;
...
%-------------------------------------------------------------------
function [ac, stats] = processEpoch_val(ac, ae, imdb, params, mode, f_val)
%------------------------------------------------------------------   
subset = params.(mode);
num = 0;
stats.total_mse = 0;
...
for n=1:params.batch_size:numel(subset)
    ...
    im = zscore(im);
    ae_mse = mse(im, output); 
    stats.total_mse = stats.total_mse + ae_mse;
